...
// leg_length = 0.10:0.02:0.40 m. Maximum relative error at a fit node is
// 1.03%. The polynomial variable is s = (leg_length - 0.25) / 0.15, so s
// stays in [-1, 1].
// State: [theta, dtheta, x, dx, phi, dphi].
// Input: [total wheel torque, total leg-angle torque].
// Coefficients are ordered from cubic to constant for Horner evaluation.
